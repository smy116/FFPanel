...
const keys = computed(() => {
  const values = new Set([
    ...
    ...Object.keys(props.decision.requested || {}),
    ...Object.keys(props.decision.effective || {}),
  ]);
...
<template>
  <div class="decision-panel">
    <div class="decision-toolbar">
      <b>{{ hasFallback ? "参数决策 · 已自动退回" : "参数决策" }}</b
      ><label><input v-model="changedOnly" type="checkbox" />只看变化项</label>
    </div>
    <div class="decision-table">
      <div class="decision-head">
        <span>字段</span><span>源参数 Source</span
        ><span>用户参数 Requested</span><span>实际参数 Effective</span>
      </div>
      <div v-for="key in keys" :key="key" class="decision-row">
        <b>{{ key }}</b
        ><span>{{ value(decision.source, key) }}</span
        ><span>{{ value(decision.requested, key) }}</span
        ><span class="effective">{{ value(decision.effective, key) }}</span>
      </div>
      <p v-if="!keys.length" class="muted-copy">没有自动调整的参数。</p>
    </div>
    <div v-if="decision.reasons.length" class="reason-list">
      <span
        v-for="(reason, index) in decision.reasons"
        :key="reason.code + reason.field + index"
        >{{ reason.message }}</span
      >
    </div>
    <div v-if="decision.ffmpegArgv?.length" class="argv">
      <b class="argv-heading">FFmpeg 参数</b
      ><code>{{ decision.ffmpegArgv.join(" ") }}</code>
    </div>
  </div>
</template>
