...
const video: TaskFile = {
  ...
  parameterDecision: { source: { bitrateKbps: 228 }, requested: { bitrateKbps: 2000, rateControl: 'vbr' }, effective: { bitrateKbps: 228, rateControl: 'vbr' }, reasons: [], ffmpegArgv: ['ffmpeg', '-i', 'movie.mp4'] },
...
describe('file table details', () => {
  ...
  it('keeps the headers visible and opens file details in a modal', async () => {
    ...
    expect(document.body.textContent).toContain('rateControl')
    expect(document.body.textContent).toContain('vbr')
    expect(document.body.textContent).toContain('FFmpeg 参数')
    expect(document.body.textContent).toContain('ffmpeg -i movie.mp4')
    expect(document.body.textContent).toContain('FFmpeg 输出')
    ...
    expect(document.querySelector('.file-detail-scroll .file-detail-meta')).not.toBeNull()
    expect(document.querySelector('.file-detail-scroll .file-detail-path')).not.toBeNull()
    expect(document.querySelector('[id^="reka-dialog-description"]')).toBeNull()
  })
})
