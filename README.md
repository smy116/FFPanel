...
正式输出先写入 `.part` 临时文件，成功并完成校验后再提交到目标路径；已有目标文件不会被静默覆盖。容器或进程异常退出后，正在运行的任务会标记为 `interrupted`，可从文件级安全检查点 Retry。Retry 不是从 FFmpeg 中间帧继续编码。
...
- 已完成文件保持完成，不重复处理。
- 转码阶段中断或产物校验失败的文件重新执行 ffprobe、参数决策和转码。
