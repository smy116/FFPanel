...
|                                                                               |
+-------------------------------------------------------------------------------+
| 底部极简状态栏: Rockchip MPP · Ready | Intel QSV/VAAPI · Unavailable | NVIDIA NVENC · Unavailable | Rclone · 3 Remotes |
+-------------------------------------------------------------------------------+
```

### 3.1 顶部导航栏 (Navbar)

- **左侧**：FFPanel 品牌标识（内置芯片小图标）。
- **中间 Tab 切换**：
  - `新建转码任务`（含快捷指示器）
  - `任务清单`（右侧显示待处理任务角标 Badge；后端只有 1 个 FFmpeg 转码槽位，但另有 1 个上传槽位，因此可能同时存在“任务 A 上传 + 任务 B 转码”的两条活动链路）。
- **右侧状态栏**：简要系统心跳（通过 SSE 推送 Rockchip、Intel、NVIDIA 硬件状态，以及 CPU 与内存占用率）；三类硬件在底部始终展示，未就绪时标记为 `Detecting`、`Partial` 或 `Unavailable`。
...
- **转码自动退回开关**：默认开启。
  ...
  - 方案列表只展示当前目标编码实际可用的档位；设备未就绪或不支持目标编码的方案直接隐藏。能力更新导致当前选择失效时，自动切换到优先级最高的可用档位。
...
- **目标码率控制 (Bitrate Allocation)**：
  ...
  - 高级设置折叠面板（Accordion）：码率控制模式（默认 VBR；硬件编码映射为 `-rc_mode VBR/CBR`，CPU 编码通过码率上下限实现）、音频流策略（默认 Copy 音轨 `-c:a copy`）、字幕处理策略。GOP 不显式指定，由编码器自行决定。字幕编码/封装方式根据目标容器动态决定，不将 `-c:s mov_text` 作为所有容器的固定规则。
...
2.  **卡片主体 (Progress & Current File)**：
    ...
    - **活动流水线详情 (Active Pipeline)**：允许同时展示两个并列/上下排列的子卡片：
      ...
      - 当上传与转码同时发生时，在两张子卡片之间显示轻量 `Pipeline` 标识。
...
5.  **双槽位流水线可视化**：
    - 系统状态区分别显示 `Transcode: 0/1 | Upload: 0/1`。
